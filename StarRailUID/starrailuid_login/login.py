...
async def page_login(bot: Bot, ev: Event):
    ...
    if isinstance(existing, dict):
        ...
        return await bot.send(
            ...
            f"请在浏览器中打开: {url}/sr/login/{user_token}\n"
            f"登录地址10分钟内有效",
            at_sender=at_sender,
        )
    ...
    url = get_server_url()
    login_url = f"{url}/sr/login/{user_token}"
...
async def _post_process_cookie(user_id: str, bot_id: str, cookie_str: str):
    ...
    2. Sets sr_uid=None because get_mihoyo_bbs_info() returns empty.
    ...
    if not mys_id:
        logger.warning("[sr登录] Could not extract account_id from cookie")
        return
    ...
    try:
        ...
        logger.info(f"[sr登录] Patched cookie for mys_id={mys_id}")

        # 2) Link sr_uid if missing
        cursor.execute(
            "SELECT sr_uid FROM gsuser WHERE mys_id = ?", (mys_id,)
        )
        row = cursor.fetchone()
        if row and not row[0]:
            # sr_uid is NULL — find an unlinked one from gsbind
            cursor.execute(
                "SELECT sr_uid FROM gsbind WHERE user_id = ? AND bot_id = ?",
                (user_id, bot_id),
            )
            bind_row = cursor.fetchone()
            if bind_row and bind_row[0]:
                sr_uids = bind_row[0].split("_")
                # Find an sr_uid not already claimed by another gsuser row
                for sr_uid in sr_uids:
                    cursor.execute(
                        "SELECT id FROM gsuser WHERE sr_uid = ?", (sr_uid,)
                    )
                    if not cursor.fetchone():
                        region = SR_SERVER.get(
                            sr_uid[0], "prod_official_usa"
                        )
                        cursor.execute(
                            "UPDATE gsuser SET sr_uid = ?, sr_region = ? WHERE mys_id = ?",
                            (sr_uid, region, mys_id),
                        )
                        logger.info(
                            f"[sr登录] Linked sr_uid={sr_uid} to mys_id={mys_id}"
                        )
                        break

        conn.commit()
    except Exception as e:
        logger.error(f"[sr登录] post-process failed: {e}")
    finally:
        conn.close()
...
async def process_cookie(auth: str, cookie_str: str) -> dict:
    ...
    try:
        ...
        if ok_count >= 1:
            # Fix for international users: patch cookie + link sr_uid
        ...
        logger.error(f"[sr登录] Cookie处理失败: {e}")
        entry["status"] = "error"
...
@app.get("/sr/cookie-guide")
async def sr_cookie_guide():
    pdf_path = TEMPLATES_DIR / "HoYoLab_Cookie_Guide.pdf"
    return FileResponse(pdf_path, media_type="application/pdf")


@app.get("/sr/login/{auth}")
async def sr_login_index(auth: str):
    temp = cache.get(auth)
    if temp is None:
        html_path = TEMPLATES_DIR / "sr_404.html"
        html = html_path.read_text(encoding="utf-8")
        return HTMLResponse(html)

    url = get_server_url()
    html_path = TEMPLATES_DIR / "sr_login.html"
    html_template = html_path.read_text(encoding="utf-8")

    # Simple template replacement (no Jinja2 dependency)
...
@app.post("/sr/login/cookie")
async def sr_login_cookie(data: CookieLoginModel):
    ...
    # Basic validation: must contain at least one known key
    known_keys = [
...
async def _do_auto_login(auth: str, email: str, password: str):
    ...
    try:
        ...
        async def _solver(session_mmt):
            ...
            logger.info("[sr登录] Geetest captcha triggered, waiting for user...")

            # Poll cache for up to 120 seconds until frontend submits solution
            for _ in range(120):
                await asyncio.sleep(1)
                current = cache.get(auth)
                if not current:
                ...
                if solution:
                    logger.info("[sr登录] Geetest solution received from frontend")
        ...
        error_msg = str(e)
        logger.error(f"[sr登录] 自动登录失败: {error_msg}")
...
@app.post("/sr/login/auto")
async def sr_login_auto(data: AutoLoginModel):
    """Start auto-login as a background task."""
    entry = cache.get(data.auth)
    if entry is None:
        return {"success": False, "msg": "会话已过期，请重新发送登录命令"}

    # Kick off login in background — frontend will poll /sr/login/status
    asyncio.create_task(_do_auto_login(data.auth, data.email, data.password))
    return {"success": True, "msg": "正在登录...", "pending": True}


@app.get("/sr/login/status/{auth}")
async def sr_login_status(auth: str):
...
@app.post("/sr/login/geetest")
async def sr_login_geetest(data: GeetestSolverModel):
    ...
    logger.info("[sr登录] Geetest solution stored, background task will pick it up")
    return {"success": True}
